...
const BlogPostCard = ({ post, onClick }) => {
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`Blog post card for ${post.title}`}
      onClick={() => onClick(post)}
      onKeyDown={(e) => {
        if (e.key === "Enter") onClick(post);
      }}
    ...
    >
      ...
      <div className="p-6 bg-[#181e29] flex-grow flex flex-col">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xl font-bold text-blue-400 group-hover:text-[#89FFAA] group-focus-visible:text-[#89FFAA] transition-colors duration-200 leading-snug flex-grow">
            {post.title}
          </h2>
        </div>

        <p className="text-slate-400 mb-4 text-base line-clamp-3">
          {post.excerpt}
        </p>
